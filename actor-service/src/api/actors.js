 ...
 */
hash.set('GET /', async function listUsers (req, res, params) {
  try {
    if ('0' in req.query)
 ...
 * @return {promise} response, an object with the information of the user including refresh and access tokens.
 */
hash.set('GET /:id', async function getUser (req, res, params) {
  try {
    return send(res, 200, mainUtils.getActor(params.id))
  } catch (err) {
    return send(res, 500, { error: err.message })
  }
})
